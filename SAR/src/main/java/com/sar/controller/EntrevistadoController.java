...
public class EntrevistadoController implements Serializable {
    ...
    public String agregarEntrevista() {
        ...
        req.execute("PF('dates').hide();");
                    FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "AVISO", "SE AGREGÓ A ENTREVISTADOS EXITOSAMENTE"));
    ...
    public String modify()
    {
        try
        {
            ...
            searchPostulante();
            System.out.println(p.getCedula()); 
        for(Entrevistados entrevista : entrevistaFacade.findAll()){
            ...
            this.e.setPostulante(this.p);
            this.entrevistaFacade.edit(this.e);
            this.p = new Postulante();
            this.e = new Entrevistados();
            RequestContext req = RequestContext.getCurrentInstance();
            req.execute("PF('dates').hide();");
   ...
   public void checkEntrevista(Postulante pos){
      for (Entrevistados i : entrevistaFacade.findAll())
        {
            if (i.getPostulante().getCedula().equals(pos.getCedula()))
            {
                FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "INFO!", "YA FUE ENTREVISTADO."));
                break;
            }
        }
